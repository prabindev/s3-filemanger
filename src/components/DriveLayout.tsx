...
export default function DriveLayout({ children }: { children: React.ReactNode }) {
  ...
  const NavItem = ({ icon: Icon, label, href, active }: any) => (
    <Link 
      href={href}
      className={`flex items-center gap-4 px-4 py-2 rounded-full mb-1 transition-colors ${
        active 
    ...
    >
      <Icon size={20} className={active ? "text-[#001D35] dark:text-[#C2E7FF]" : ""} />
  ...
  return (
    <div className="flex h-screen bg-[#F8F9FA] dark:bg-[#131314] overflow-hidden text-[#1F1F1F] dark:text-[#E3E3E3] font-sans">
      ...
      <aside className={`fixed md:static inset-y-0 left-0 z-40 w-64 bg-[#F8F9FA] dark:bg-[#131314] transform transition-transform duration-200 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div className="p-4 h-full flex flex-col">
          ...
          <div className="px-2 mb-6">
            <button className="flex items-center gap-4 bg-white dark:bg-[#37393B] shadow-[0_1px_2px_0_rgba(60,64,67,0.3),0_1px_3px_1px_rgba(60,64,67,0.15)] dark:shadow-none hover:shadow-[0_1px_3px_0_rgba(60,64,67,0.3),0_4px_8px_3px_rgba(60,64,67,0.15)] hover:bg-[#F8F9FA] dark:hover:bg-[#474A4D] pl-4 pr-6 py-3.5 rounded-[16px] transition-all">
          ...
          <nav className="flex flex-col gap-0.5 flex-1 overflow-y-auto hide-scrollbar pl-2 pr-4">
            ...
            <NavItem icon={HardDrive} label="My Drive" href="/dashboard" active={pathname.includes("/bucket")} />
            <NavItem icon={Users} label="Shared with me" href="#" />
            <NavItem icon={Clock} label="Recent" href="#" />
            <NavItem icon={Star} label="Starred" href="#" />
            
            <div className="my-3 mx-4 border-t border-[#E0E0E0] dark:border-[#444746]"></div>
            
            <NavItem icon={AlertCircle} label="Spam" href="#" />
            <NavItem icon={Trash2} label="Trash" href="#" />
            <NavItem icon={Cloud} label="Storage" href="#" />
            
            <div className="px-4 py-2 mt-2">
              <div className="w-full bg-[#E0E0E0] dark:bg-[#444746] rounded-full h-1 mb-2">
                <div className="bg-[#0B57D0] dark:bg-[#A8C7FA] h-1 rounded-full" style={{ width: '15%' }}></div>
              </div>
              <p className="text-[13px] text-[#444746] dark:text-[#C4C7C5]">15 GB of 100 GB used</p>
              <button className="mt-3 text-[14px] font-medium text-[#0B57D0] dark:text-[#A8C7FA] border border-[#747775] dark:border-[#8E918F] rounded-full px-5 py-1.5 hover:bg-[#0B57D0]/5 dark:hover:bg-[#A8C7FA]/5 transition-colors">
                Get more storage
              </button>
            </div>
          </nav>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[#F8F9FA] dark:bg-[#131314] overflow-hidden relative">
        ...
        <main className="flex-1 overflow-y-auto bg-white dark:bg-[#1E1F20] rounded-[24px] mx-2 sm:mx-4 mb-2 sm:mb-4 shadow-sm border border-[#E0E0E0] dark:border-[#444746]">
          {children}
        </main>
      </div>
